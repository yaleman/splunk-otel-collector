...
func TestOTLPSinkDeploymentManifest(t *testing.T) {
	...
	dep := OTLPSinkDeployment{tc: testutils.NewTestcase(t)}
	sb, err := newSBRest(t.Logf)
	require.NoError(t, err)
	dep.sb = sb
	dep.apiEndpoint = "some.api.endpoint"
	dep.otlpEndpoint = "some.otlp.endpoint"
...
`
	...
	require.Equal(t, expectedManifests, dep.manifests())
}
