...
func TestPortEndpointProperties(t *testing.T) {
	...
	configMap, configMapManifest := configToConfigMapManifest(t, namespace)
	...
	redisName := createRedis(cluster, "target.redis", namespace, serviceAccount)
		...
		ctx, &corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Annotations: map[string]string{
					"receiver-creator.collector.opentelemetry.io/smartagent__redis-monitor.rule":               `type == "port"`,
					"receiver-creator.collector.opentelemetry.io/smartagent__redis-monitor.config.type":        "collectd/redis",
					"receiver-creator.collector.opentelemetry.io/smartagent__redis-monitor.res_attrs.attr.one": "attr_one_value",
				},
				Labels: map[string]string{
					"receiver-creator.collector.opentelemetry.io/smartagent__redis-monitor.res_attrs.attr.two": "attr_two_value",
				},
				Name: name,
			},
			Spec: corev1.PodSpec{
...
func configToConfigMapManifest(t testing.TB, namespace string) (name, manifest string) {
	config := `extensions:
  k8s_observer:
...
`
	...
	configStore := map[string]any{"config": config}
	configYaml, err := yaml.Marshal(configStore)

	cm := manifests.ConfigMap{
		Name: "collector.config", Namespace: namespace, Data: string(configYaml),
	}
	cmm, err := cm.Render()
	require.NoError(t, err)
	return cm.Name, cmm
